...
- **TDD를 의무화한다.** 코드 변경 시 반드시 관련 테스트를 먼저 확인/수정한다.
- **동일 배치에서 테스트와 구현을 동시에 작성하지 않는다.**

## 코드 품질 검색 패턴

Phase 1 ANALYZE에서 다음 패턴으로 위반 사항을 스캔한다:

| Target | Pattern |
|--------|---------|
| `var` 사용 | `rg '\bvar\b' lib/src/ --type dart` |
| 누락된 `const` | `rg '(?<!const\s)\b(Text|EdgeInsets|Size|Offset|Color|BoxConstraints)\(' lib/src/ --type dart` |
| 긴 함수 (30줄+) | 수동 스캔 또는 AST 분석 |
| 중첩 깊이 (3+) | 수동 스캔 |
| TODO/FIXME | `rg 'TODO\|FIXME\|HACK' lib/src/ --type dart` |
| 요청되지 않은 주석 | `rg '^\s*//' lib/src/ --type dart` (공개 API dartdoc 제외) |
| 누락된 명시적 타입 | `rg '\bfinal\s+\b' lib/src/ --type dart` (타입 없는 final) |
...
2. 코드 품질 검색 패턴 실행 (SKILL.md §코드 품질 검색 패턴 참조):
   - `rg '\bvar\b' lib/src/ --type dart`
   - `rg 'TODO|FIXME|HACK' lib/src/ --type dart`
   - 함수 길이, 중첩 깊이 수동 스캔
3. 코드베이스 스캔 → 3~5개 개선점 발견
   - Scan Level 1부터 시작, 개선점 없으면 다음 레벨로 확대
   - 검사 항목: 함수 길이(>30줄), 중첩 깊이(>3), var 사용, 누락된 const, 성능
4. 개선점 우선순위 정렬 (영향도 × 난이도)
...
각 개선점을 순차적으로:
1. **TDD 원칙 준수**: 코드 변경 시 관련 테스트를 먼저 확인/수정
2. 설계에 따라 코드 수정
3. `dart format .` 실행
4. `dart analyze` 실행 — 0 issues 필수 (에러, 경고, 힌트 모두)
5. `dart test` 실행 — 전체 통과 필수
6. git commit (커밋 메시지에 문제+해결+결과 포함)
   - 예: `perf: cache TextStyle in RenderDropdownMenu — eliminate per-item allocations`
7. analyze/test 실패 시 수정 후 재시도 (최대 3회, 그 후 스킵)
8. **Flutter Engineer (실행) 체크리스트 검증** (personas.md 참조)
