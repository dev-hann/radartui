...
class KeyParser {
  static KeyEvent parse(List<int> rawData) {
        ...
        case 49: // F1 (ESC [ 1 P) - this is a common one, but others are ESC O P/Q/R/S
          ...
          if (rawData.length >= 4 &&
            ...
            switch (rawData[2]) {
              ...
              case 55:
                return const KeyEvent(code: KeyCode.f5); // ESC [ 15 ~ (F5)
              case 56:
                return const KeyEvent(code: KeyCode.f6); // ESC [ 17 ~ (F6)
              case 57:
                return const KeyEvent(code: KeyCode.f7); // ESC [ 18 ~ (F7)
              case 48: // ESC [ 20 ~ (F9)
                if (rawData.length >= 5 && rawData[4] == 50)
    ...
    if (rawData.length == 1 && rawData[0] >= 1 && rawData[0] <= 26) {
      isCtrl = true;
      final charCode =
          rawData[0] + 64; // Convert to ASCII char code (e.g., 1 -> A, 2 -> B)
      return KeyEvent(
        code: KeyCode.char,
