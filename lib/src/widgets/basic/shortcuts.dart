...
import '../../foundation/utils.dart' show mapEquals, iterableEquals;
...
class Shortcuts extends StatefulWidget {
  ...
  static Intent? _matchShortcut(KeyEvent event, _ShortcutsScope scope) {
    for (final key in scope.shortcuts.keys) {
      if (key.accepts(event)) return scope.shortcuts[key];
    }
    return null;
  }
...
class _ActionsScope extends InheritedWidget {
  ...
  bool updateShouldNotify(_ActionsScope oldWidget) {
    return !iterableEquals(actions.keys, oldWidget.actions.keys);
  }
}
